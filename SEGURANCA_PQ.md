...
| Selo na conversa | `src/components/security/PqBadge.tsx` | Verde "Pós-quântica" quando a conversa está protegida, âmbar "Sem PQ" quando não está; abre a explicação e os dois códigos de verificação |
| Página de segurança | `src/pages/Security.tsx` (rota `/seguranca`) | Explica a tecnologia, o que protege e o que ainda não protege |
| Menu "Mais" | `src/components/Layout/AppLayout.tsx` | Item "Segurança" |
| Tela de login | `src/pages/Auth.tsx` | Faixa de confiança antes do cadastro |
...
`src/lib/pq/e2e.integration.test.ts` — 16 casos de **integração**, com um
Supabase de mentira que replica as tabelas reais. Exercita exatamente o que
`Messages.tsx` chama: publicação no login, envio 1:1, envio em grupo, leitura
do histórico em lote. Cobre também:

- conversa completa de 20 mensagens nos dois sentidos;
- decifragem repetida (o que acontece a cada re-render);
- mensagem antiga em texto puro continuando legível;
- **servidor malicioso** trocando a chave ML-KEM (o cliente se recusa a cifrar)
  e trocando o par inteiro (o código de segurança muda e o destinatário
  legítimo não consegue abrir — o ataque é ruidoso);
- cache de bundles (1 leitura de perfil para 10 mensagens, não 10);
- uma mensagem corrompida no meio do histórico não derruba as outras.

```bash
npm test                       # suíte completa (63 casos)
npx vitest run src/lib/pq      # só a camada criptográfica
```

### Dois defeitos que esta suíte encontrou (e que já foram corrigidos)

1. **Memo de publicação sem prazo.** `publishedFor` marcava "já publiquei" para
   sempre. Se a linha do perfil fosse apagada no servidor (restauração de
   backup, migração), o cliente nunca republicaria e ficaria invisível para
   quem quisesse cifrar para ele — até recarregar a página. Agora o memo expira
   em 30 minutos.
2. **Identidade só em memória.** Se o navegador esvaziasse o `localStorage` no
   meio da sessão (cota, limpeza de dados do site), o cache em memória
   escondia o problema e a identidade era perdida no próximo carregamento —
   levando junto o acesso ao próprio histórico. Agora `ensureIdentity` percebe
   que o storage sumiu e regrava as seeds.
